...
:-write('Ferrari SF-24 ready.').

eve(start_race):-write('Ferrari LAP 1: LIGHTS OUT AND AWAY WE GO!'),a(message(mclaren,send_message(lap_1_done,ferrari))).

eve(lap_2_done):-write('Ferrari LAP 3: Box box box! Undercut strategy!'),a(message(mclaren,send_message(ferrari_pitted,ferrari))).

eve(mclaren_pitted):-write('Ferrari LAP 4: McLaren out of pits. Defending position!'),a(message(mclaren,send_message(final_lap,ferrari))).

eve(finish):-write('Ferrari LAP 5: CHEQUERED FLAG! Ferrari wins the Grand Prix! FORZA FERRARI!').
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_163683,var_Ontology,_163687),_163677),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_163721)),a(message(var_Ag,propose(var_A,[_163721],var_AgI))),retractall(ext_agent(var_Ag,_163759,var_Ontology,_163763)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_163557,var_Ontology,_163561),_163551),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_163627,var_Ontology,_163631)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_163445,var_Ontology,_163449),_163439),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_163501,var_Ontology,_163505)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_163009),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_163043),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_162825).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_162673),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_162621),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_162497,_162499)),clause(agente(_162519,_162521,_162523,var_S),_162515),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_162291,_162293,_162295,var_S),_162287),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_162391,[]),repeat,read(_162391,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_162391).
