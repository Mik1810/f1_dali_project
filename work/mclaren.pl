...
:-write('McLaren MCL38 ready.').

eve(lap_1_done):-write('McLaren LAP 2: Ferrari done lap 1. Norris closing the gap!'),a(message(ferrari,send_message(lap_2_done,mclaren))).

eve(ferrari_pitted):-write('McLaren LAP 3: Ferrari in the pits! McLaren takes virtual lead! McLaren also boxes!'),a(message(ferrari,send_message(mclaren_pitted,mclaren))).

eve(final_lap):-write('McLaren LAP 5: FINAL LAP! DRS attack on Ferrari! Ferrari holds! McLaren P2. Great race!'),a(message(ferrari,send_message(finish,mclaren))).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_163127,var_Ontology,_163131),_163121),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_163165)),a(message(var_Ag,propose(var_A,[_163165],var_AgI))),retractall(ext_agent(var_Ag,_163203,var_Ontology,_163207)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_163001,var_Ontology,_163005),_162995),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_163071,var_Ontology,_163075)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_162889,var_Ontology,_162893),_162883),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_162945,var_Ontology,_162949)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_162453),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_162487),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_162269).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_162117),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_162065),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_161941,_161943)),clause(agente(_161963,_161965,_161967,var_S),_161959),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_161735,_161737,_161739,var_S),_161731),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_161835,[]),repeat,read(_161835,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_161835).
